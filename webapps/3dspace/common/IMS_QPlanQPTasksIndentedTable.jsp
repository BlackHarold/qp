...
<script type="text/javascript" src="./scripts/IMS_CheckTask.js"></script>
<script>
  ...
  function findFrameByUrl(w, urlPart) {
    const frames = w.document.getElementsByTagName("iframe");
    for (let i = 0; i < frames.length; i++) {
      const childW = frames[i].contentWindow;
      if (childW.location.href.indexOf(urlPart) > 0) {
        return childW;
      }
      const foundFrame = findFrameByUrl(childW, urlPart);
      if (foundFrame != null) {
        return foundFrame;
      }
    }
  ...
  function handleMutationList(mutationsList, observer) {
    mutationsList.forEach(function (mutation) {
      if (mutation.type === "childList") {
        for (let i = 0; i < mutation.addedNodes.length; i++) {
          const node = mutation.addedNodes[i];
          if (typeof (node.querySelectorAll) !== "undefined") {
            const inputs = node.querySelectorAll("input[id^='rmbrow'], input[name='chkList']");
            for (let j = 0; j < inputs.length; j++) {
              $(inputs[j]).click(function () {
                const rightWindow = findFrameByUrl(window.parent, "expandQPlanQPTreeObject");
                if (rightWindow != null) {
                  rightWindow.postMessage(
                          {
                            IMS_Action: "refresh",
                            IMS_CheckedRowObjectIds: getCheckedRowObjectIds()
                          },
                          "*");
                }
              });
